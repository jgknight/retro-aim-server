...
// flag, URL, length
func (s AdminService) ConfirmRequest(_ context.Context, frame wire.SNACFrame) wire.SNACMessage {
...
func (s AdminService) InfoQuery(_ context.Context, sess *state.Session, frame wire.SNACFrame, body wire.SNAC_0x07_0x02_AdminInfoQuery) wire.SNACMessage {
	...
	if _, hasRegStatus := body.TLVRestBlock.Slice(wire.AdminTLVRegistrationStatus); hasRegStatus {
		return getAdminInfoReply(wire.AdminTLVRegistrationStatus, wire.AdminInfoRegStatusFullDisclosure) // todo: get from session/db

	} else if _, hasEmail := body.TLVRestBlock.Slice(wire.AdminTLVEmailAddress); hasEmail {
		return getAdminInfoReply(wire.AdminTLVEmailAddress, sess.IdentScreenName().String()+"@aol.com") // todo: get from session/db

	} else if _, hasNickName := body.TLVRestBlock.Slice(wire.AdminTLVScreenNameFormatted); hasNickName {
		return getAdminInfoReply(wire.AdminTLVScreenNameFormatted, sess.DisplayScreenName().String())
...
func (s AdminService) InfoChangeRequest(ctx context.Context, sess *state.Session, frame wire.SNACFrame, body wire.SNAC_0x07_0x04_AdminInfoChangeRequest) wire.SNACMessage {
	...
	var validateProposedName = func(name state.DisplayScreenName) (bool, uint16) {
		if len(name) > 16 {
	...
	if sn, hasScreenNameFormatted := body.TLVRestBlock.Slice(wire.AdminTLVScreenNameFormatted); hasScreenNameFormatted {
		...
		s.accountManager.UpdateDisplayScreenName(proposedName)
		s.buddyUpdateBroadcaster.BroadcastBuddyArrived(ctx, sess)
		return replyMessage(wire.AdminTLVScreenNameFormatted, proposedName.String())
	}
	return wire.SNACMessage{
		Frame: wire.SNACFrame{
			FoodGroup: wire.Admin,
